...
class PredicateGenerator(Iterator[str], Iterable[str]):
    ...
    def clean_copy(self):
        ...
        :rtype: VariableGenerator
        """
        return PredicateGenerator(self._all_avoid_terms, self.name_format)

    def has_generated(self, predicate):
        """
        Checks if the predicate could have been generated by this generator.

        :param predicate: the predicate
        :type predicate: Predicate or str
        :return: `True`, if it could have been generated; otherwise, `False`
        :rtype: bool
        """
        if isinstance(predicate, Predicate):
            predicate = predicate.name
        match = self._name_regex.fullmatch(predicate)
        if match is None:
            return False
        return int(match.groups()[0]) < self._current_index
