...
CONFIGURATION = "configuration_meta.yaml"
PROGRAM = "kinship.pl"
...
CLAUSES = {
    HornClause(
        Atom(PARENT, Variable("X0"), Variable("X1")),
        Literal(Atom(Predicate("mother", 2), Variable("X0"), Variable("X1")))),
    HornClause(
        Atom(PARENT, Variable("X0"), Variable("X1")),
        Literal(Atom(Predicate("father", 2), Variable("X0"), Variable("X1")))),
}


class TestMetaStructureLearning(unittest.TestCase):
