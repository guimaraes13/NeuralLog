...
class MetaRevisionOperator(ro.RevisionOperator):
    ...
    def initialize(self):
        super().initialize()
        self._meta_clauses: List[HornClause] = self._read_program()
        if self.tree_theory is None:
            self.perform_operation = self.perform_operation_on_example
        else:
    ...
    def perform_operation_on_example(self, targets, minimum_threshold=None):
        """
