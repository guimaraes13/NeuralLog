...
def get_file_source(clause):
    ...
    :rtype: str
    """
    if isinstance(clause.provenance.start.source[0], NeuralLogLexer):
        inputStream = clause.provenance.start.source[0].inputStream
    ...
    if logger.isEnabledFor(logging.WARNING):
        for older_clause in input_clauses:
            if clause == older_clause:
                start_line = clause.provenance.start.line
                start_column = clause.provenance.start.column
                clause_filename = get_file_source(clause)

                old_start_line = older_clause.provenance.start.line
                old_start_col = older_clause.provenance.start.column
                old_clause_filename = get_file_source(older_clause)
                logger.warning(
